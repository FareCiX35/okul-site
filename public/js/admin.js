(() => {
  ...
  const api = async (path, options = {}) => {
    ...
    if (!res.ok) {
      ...
      throw new Error(msg.error || "��lem ba�ar�s�z");
    }
    return res.json();
  };
          ...
          return `
            <div class="collection-item">
              ...
              <div class="item-actions">
                <button data-action="edit" data-collection="${escapeHtml(name)}" data-id="${escapeHtml(item.id)}">D�zenle</button>
  ...
  const attachFormListeners = () => {
    ...
    document.querySelectorAll(".collection-list").forEach((list) => {
      list.addEventListener("click", async (event) => {
        ...
        if (action === "delete") {
          const ok = confirm("Silmek istedi�inize emin misiniz?");
          if (!ok) return;
          try {
